...
check(
    "engage derives provisions",
    set(f.engaged_provisions) == {"ppe_reg4", "ppe_reg10", "mhswr_reg3", "hswa_s2", "ppe_reg7"},
)
...
check(
    "shared action stems from both findings",
    set(reassess[0].from_findings) == {"obs_001", "obs_002"},
)

r1 = consolidate([head()], kb)
prov = next(a for a in r1.actions if a.action_id == "ca_provide_head").discharges_here
check(
    "specific duties lead general",
    [p.duty_scope.value for p in prov]
    == sorted([p.duty_scope.value for p in prov], key=lambda s: 0 if s == "specific" else 1),
)

r1 = consolidate([head()], kb)
check(
    "maintenance action closes ppe_reg7 gap", not any(g.provision.id == "ppe_reg7" for g in r1.gaps)
)
check(
    "maintenance action appears in plan", any(a.action_id == "ca_maintain_ppe" for a in r1.actions)
)
